...
pub fn spawn_automatic_projectile( 
    ...
    sprite_path: impl AsRef<str>, // Changed from &'static str
    size: Vec2,
    color: Color,
    lifetime_secs: f32,
    // Bouncing parameters
                ...
                if let Some(tether_comp_on_projectile) = opt_tether_projectile {
                    ...
                    if let Some(bounces) = proj_stats.bounces_left {
                        if bounces > 0 && !proj_stats.has_bounced_this_frame {
                            ...
                            let reflection_dir = (proj_gtransform.translation().truncate() - horror_gtransform.translation().truncate()).normalize_or_zero(); // Changed normalize_or_else to normalize_or_zero
